...
export const TableHeader: FC<{ children: JSX.Element[] }> = ({ children }) => {
    return (
        <thead className='text-white text-left bg-primary'>
            <tr className='[&>th]:px-4 [&>th]:py-2'>{children}</tr>
        </thead>
    );
};
...
export const TableRow: FC<{ children: JSX.Element[] }> = ({ children }) => {
    return <tr className='[&>td]:px-4 [&>td]:py-3 border-b-[#f2f2f2] border-b-[1px] even:bg-[#f4f4f4] hover:bg-[#f3f3f3]'>{children}</tr>;
};

export const TableHead: FC<{ children: any; className?: string }> = ({ children, className }) => <th className={className}>{children}</th>;
export const TableColumn: FC<{ children: any; className?: string }> = ({ children, className }) => <td className={[className, ''].join(' ')}>{children}</td>;
